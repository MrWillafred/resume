...
{
    internal class TuneContenReply : Comand
    {
        public override string[] Names { get; set; } = new string[] { "На что меняем контент?" };
            ...
            using (ApplicationContext db = new ApplicationContext())
            {
                ...
                foreach (var user in users)
                {
                    if (user.Statuses.Title == "Admin")
                    {
                        ...
                        var customer = db.File_To_Send
                                .Where(c => c.Id == DataBase.contentID)
                                .FirstOrDefault();

                        // Внести изменения
                        customer.File = message.Text;
