...
{
    internal class ReplyContent : Comand
    {
        public override string[] Names { get; set; } = new string[] { "Введите ID контента для изменения:" };
        ...
        public override async void Execute(Message message, TelegramBotClient client)
        {
            using (ApplicationContext db = new ApplicationContext())
            {
                ...
                foreach (var user in users)
                {
                    ...
                    {
                        DataBase.contentID = Convert.ToInt32(message.Text);

                        var text = db.Message_Text.ToArray();
                        await client.SendTextMessageAsync(message.Chat.Id, "На что меняем контент?", replyMarkup: new ForceReplyMarkup { Selective = true });
                    }
